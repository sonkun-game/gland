...
export async function getShopsFromApi(status, name) {
  ...
  if (false) {
    try {
      const response = await axios.get('http://103.142.26.40:8080/Total/api-store/all?status=' + status + '&name=' + name, {
        headers: {
...
export async function createShop(name, address, phone, status) {
  axios({
    method: 'post',
    url: 'http://103.142.26.40:8080/Total/api-store/create',
    responseType: 'json',
    data: {
      name: name,
      address: address,
...
export async function editShop(name, address, phone, status, id) {
  axios({
    method: 'put',
    url: 'http://103.142.26.40:8080/Total/api-store/edit/' + id,
    responseType: 'json',
    data: {
      name: name,
      address: address,
