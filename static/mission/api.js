...
export async function getAllMissionPaging(storeId) {
  let url = "https://api.gland84.io.vn:8447/Spa/api-mission/all?pageNum=0&storeId=" + storeId;
  try {
    const response = await axios.get(url, {
      headers: {
        'Content-Type': 'application/json',
...
export async function createMissionAll(storeId, misionTable) {
  ...
  var keyUUID = uuidv4();
  var url = 'https://api.gland84.io.vn:8447/Spa/api-mission/create?storeId=' + storeId;
