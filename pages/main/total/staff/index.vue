<template>
    <div class="p-8 text-white">
        <nav class="flex justify-between">
            <h1 class="font-bold text-4xl" :class="{'text-white':theme==='dark','text-gray-900':theme==='light'}">Nhân sự</h1>
            <ShowModal modalId="createStaffBtnId" type="custom"
                customClass="bg-blue-500 rounded-lg px-4 py-1 text-lg font-bold" title="Tạo nhân sự">
                <ModalContainer modalId="createStaffBtnId" size="xl" :isDark="theme==='dark'">
                    <ModalHeader :isDark="theme==='dark'" head="Tạo nhân sự" modalId="createStaffBtnId">
                    </ModalHeader>
                    <div class="flex flex-wrap">
                        <InputField :isDark="theme==='dark'" styleClass="p-2 w-50" id="staffName" label="Tên tài khoản"
                            placeholder="Tên nhân viên" />
                        <InputField :isDark="theme==='dark'" styleClass="p-2 w-50" id="staffEmail" label="Email"
                            placeholder="Email nhân viên" />
                        <InputField :isDark="theme==='dark'" styleClass="p-2 w-50" id="staffScript" label="Kịch bản"
                            placeholder="Kịch bản" />
                        <InputField :isDark="theme==='dark'" styleClass="p-2 w-50" id="staffPassword" label="Mật khẩu"
        ...
        <CrudTable :total-page="this.totalPage" :current-page="pageNum" style-class="w-full text-sm text-left text-gray-500">
            ...
            <tbody>
                <Row :theme="theme" v-for="(item, index) in table.body" :key="index">
                    ...
                    <Cell styleClass="px-6 py-4">{{ item.script }}</Cell>
...
export default {
    ...
    async fetch() {
        try {
            var response = await getAllStaffsPaging(this.storeId, this.pageNum);
    ...
    methods: {
        async createStaff() {
            var response = await createStaffsAll(this.storeId);
