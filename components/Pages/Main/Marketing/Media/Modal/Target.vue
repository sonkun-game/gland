                ...
                <ModalContainer modalId="createMarketingTarget" size="lg">
                    ...
                    <div class="flex items-center p-6 space-x-2 justify-end border-gray-200 rounded-b dark:border-gray-600">
                        <button :data-modal-hide="modalId"
                            ...
                            Hủy bỏ</button>
                        <button :data-modal-hide="modalId" type="button" @click="createPersonMediaConfig(storeId)"
                    ...
                    <Row styleClass="bg-white border-b" v-for="(item, index) in table.body" :key="index">
                        ...
                        <Cell styleClass="px-6 py-4 ">
                            <div class="m-auto flex justify-center">
                                <ShowModal :modalId="'editMarketingTarget' + item.id"
                                    ...
                                    <ModalContainer :modalId="'editMarketingTarget' + item.id" size="lg">
                                        ...
                                        <div
                                            ...
                                            <button :data-modal-hide="modalId" type="button"
                                                @click="editPersonMediaConfig(storeId)"
                                                class="btn btn-info text-white bg-blue-700 hover:bg-blue-800 border-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Lưu</button>
                                        </div>
                                    </ModalContainer>
                                </ShowModal>
                                <ShowModal modalId="deleteMarketingTarget" iconClass="fa-solid fa-trash text-red-500">
                                    <ConfirmModal modalId="deleteMarketingTarget" title="" type="red"
                                        message="Xóa đối tượng này" messageClass="text-lg text-rose-600">
                                    </ConfirmModal>
                                </ShowModal>
                            </div>
                        </Cell>
...
import { createPersonMediaConfig, getPersonMediaConfig, editPersonMediaConfig } from "static/marketing/media/config/person/api";
...
export default {
    ...
    methods: {
        createPersonMediaConfig(storeId) {
            createPersonMediaConfig(storeId)
        },
        formatDate(date) {
            return moment(date).format('DD/MM/YYYY HH:mm');
        },
        editPersonMediaConfig(storeId) {
            editPersonMediaConfig(storeId)
        }
    },
    async created() {
        // var listPerson = await getPersonMediaConfig(this.pageNum, this.storeId);
        // this.table.body = listPerson;
    }
}
</script>
