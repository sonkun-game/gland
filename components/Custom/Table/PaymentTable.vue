                ...
                <tbody>
                    <tr class="bg-white border-b" v-for="(item, index) in table.body" :key="index">
                        ...
                        <td class="w-24 px-6 py-4 flex text-center">
                            <button :data-modal-target="editID" :data-modal-toggle="editID" @click="setData(item, item[9])"
        ...
        <div :id="editID" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative w-full max-w-xl max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <!-- Modal body -->
                    <div class="px-4 pb-2">
                        <div class="relative z-0 w-full mb-2" v-for="(item, index) in paymentEditForm" :key="index">
                    ...
                    <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                        <button :data-modal-hide="editID" type="button"
                            ...
                            bỏ</button>
                        <button :data-modal-hide="editID" type="button" @click="editOutBill()"
...
import { getAllOutcomeBillConfig } from '@/static/billconfig/api.js';
export default {
    ...
    mounted() {
        getAllOutcomeBillConfig(this.paymentEditForm[1].selectOption);
    ...
    methods: {
        setData(editData, idElement) {
            ...
            var i = 2;
            this.paymentEditForm.forEach(item => {
                if (editData[i]) {
                    if (i === 3)
                        editData[i] = document.getElementById("typeOutId").value;
                    else if (i === 7)
                        editData[i] = document.getElementById("departmentOutId").value;
                    else if (i === 8)
                        editData[i] = document.getElementById("statusOutId").value;
                    else
        ...
        updateValue(newValue, index) {
            if (index === 1)
                this.selectedBillConfig = newValue;
            else if (index === 4)
                this.selectedDepartment = newValue;
            else if (index === 5)
                this.statusBill = newValue;
        ...
        editOutBill() {
            ...
            axios({
                ...
                responseType: 'json',
                data: {
                    payPerson: this.paymentEditForm[0].value,
                    billConfigId: document.getElementById("typeOutId").value,
                    amount: this.paymentEditForm[2].value.replace(/,/g, ''),
                    departmentId: document.getElementById("departmentOutId").value,
                    statusBill: document.getElementById("statusOutId").value,
                    note: document.getElementById("noteOutId").value
                },
                headers: {
        ...
        loadDataBillConfig() {
            this.paymentEditForm[1].selectOption = JSON.parse(localStorage.getItem("billConfigOuts"));
        }
    },
}
</script>
